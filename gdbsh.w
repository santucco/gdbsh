...
да и изучать программирование на \.{Python} нет ни времени, ни какого-либо желания.
...
Пользоваться им в целом довольно просто. \.{GDBSh} поддерживает все команды \.{GDB}, вывод которых можно перенаправить в конвейер,
в котором могут присутствовать как внешние команды, так и команды \.{GDB}.
Для использования последних есть специальнaя команда \.{args}. Она похожа на команду \.{xargs},
однако для подстановки аргументов используется способ указания номеров полей из \.{AWK}: входные данные,
разделенные пробелами, интерпретируются как отдельные поля с адресацией через \.{\$<n>}, где \.{n} - номер поля,
начиная с 1. \.{\$0} позволяет подставить все входные данные.
	...
	go func () {
		...
		if c=="args" {
			...
			for s, err:=stdr.ReadString('\n'); err==nil; s, err=stdr.ReadString('\n') {
				@<Распределить входные данные по аргументам в |cmd|@>
...
@ Входные данные будем распределять по номерам полей, аналогично \.{AWK}
Входная стока разбивается по полям с пробелом в качестве разделителя,
и подставляется в соответствующие места в |\$<n>|, где |{n}| - порядковый номер, начиная с 1
|\$0| одначает подстановку полной входной строки
@<Распределить входные данные по аргументам в |cmd|@>=
